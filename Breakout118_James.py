...
font_path = r"C:\Users\Admin\Downloads\press_start_2p\PressStart2P.ttf"
font = pygame.font.Font(font_path, 20)

font_position = (122, 300)
text_surface = font.render("Highest Score 500", True, (255, 255, 0))
...
score = 50
score_add = 100
score_update = font.render(f"Highest Score:" + str(score), True, (255, 255, 0)) 
run = True
while run:
    ...
    # -------- BALLS COLLIDING WITH BRICKS --------SCORE BOARD ---------------#
    #Big picture for every frame, did any ball hit and if so what happened?
    
 
 
    ind = 0  #“Which number brick am I on right now?”
    for brick in bricks:          #running through every brick once every frame
        if not brick["active"]:   #only concentrating on the destruction, If the brick is already destroyed, skip it, move to the next brick
            continue
        ...
        if white_circle_1.colliderect(brick["rect"]):
            ...
            if counts[ind] > 0:
                if bullet.colliderect(brick["rect"]):
                    brick["active"] = False	                      #how many times this brick has been hit			
                    value = brick.pop("rect")
            counts[ind] = counts[ind]+1             #Increase the hit count for THIS brick #break stops checking other bricks this frame
        ...
        if red_circle_2.colliderect(brick["rect"]):
            ...
            if counts[ind] >= 1:            #This brick dies on the second hit
                value = brick.pop("rect")
                brick["active"] = False                
            counts[ind] = counts[ind]+1     
            break
            
        ind = ind + 1                       #Move to the next brick index
    ...
    screen.blit(score_update, (122, 300))
